...
#if defined (MANAGE_MIME_HEADERS)
...
index.html\nhome.html");
  ...
  include_prefix = ap_document_root (r);
...
mhtml_handle_req (request_rec *r)
{
  ...
  if (req != (HTTP_REQUEST *)NULL)
    {
      req->requester = (char *)r->connection->remote_host;
      req->requester_addr = (char *)r->connection->remote_ip;
