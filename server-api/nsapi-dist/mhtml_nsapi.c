/* mhtml_nsapi.c: -*- C -*-  
    ...
    Author: Brian J. Fox (bfox@ua.com) Sun Jul  6 05:05:28 1997.
    ...
    HP-UX        ld -b t.o u.o -o test.so When compiling your code, you
		 must also use the use the +z flag to the HP C compiler. 
...
#define DBG_LOG_MSG(fun, msg) \
    if (DBG_MHTML_CGI) \
      { \
	  log_error (LOG_INFORM, fun, sn, rq, msg); \
          dbg_hdrs (fun, pb, sn, rq); \
      }

static void
dbg_hdrs (char *fun, pblock *pb, Session *sn, Request *rq)
{
  if (DBG_MHTML_CGI)
    {
      /* Server variables */
...
/* If URI starts with a string of all digits, then treat it as a Meta-HTML
   SID, remove it from the partial path, and stick it into an environment
   variable. */
NSAPI_PUBLIC int
handle_mhtml_sid (pblock *pb, Session *sn, Request *rq)
{
  register int i;
  ...
  if ((ppath != (char *)NULL) &&
      ...
      if (ppath[i] == '\0' || ppath[i] == '/')
	{
	  ...
	  /* Now stuff the value into the headers, and let the CGI
	     interface pass it to the engine. */
  ...
  /* Meta-HTML pages like getting the basic authorization information.
     Let's make sure that it gets through, by hiding the name from
     Netscape's grubby little fingers. */
  {
    char *basic_auth_string = pblock_findval ("authorization", rq->headers);

    if (basic_auth_string)
      pblock_nvinsert ("MHTML_AUTHORIZATION", basic_auth_string, rq->headers);
  }
...
NSAPI_PUBLIC int
mhtml_fixup_path (pblock *pb, Session *sn, Request *rq)
{
