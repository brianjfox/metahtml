...
HIST_ENTRY *
replace_history_entry (int which, char *line, void *data)
{
...
static int
history_search_internal (char *string, int direction, int anchored)
{
...
int
history_search (char *string, int direction)
{
...
int
history_search_prefix (char *string, int direction)
{
...
static char *
history_filename (char *filename)
{
  char *return_val = filename ? strdup (filename) : (char *)NULL;
...
int
read_history_range (char *filename, int from, int to)
{
...
int
history_truncate_file (char *fname, int lines)
{
  register int i;
  int file, chars_read;
...
static int
history_do_write (char *filename, int nelements, int overwrite)
{
  register int i;
...
int
append_history (int nelements, char *filename)
{
...
int
history_search_pos (char *string, int dir, int pos)
{
...
char *
get_history_event (char *string, int *caller_index, int delimiting_quote)
{
...
static void
rl_string_extract_single_quoted (char *string, int *sindex)
{
...
postproc_subst_rhs (void)
{
  ...
  replacement = xmalloc (new_size = subst_rhs_len + subst_lhs_len);
      ...
      if (subst_rhs[i] == '&')
	{
	  if (j + subst_lhs_len >= new_size)
	    replacement = xrealloc
	      (replacement, (new_size = new_size * 2 + subst_lhs_len));
	  strcpy (replacement + j, subst_lhs);
	  j += subst_lhs_len;
	}
      else
	{
	  ...
	  if (subst_rhs[i] == '\\' && subst_rhs[i + 1] == '&')
	    i++;
	  if (j >= new_size)
	    replacement = xrealloc (replacement, new_size *= 2);
	  replacement[j++] = subst_rhs[i];
	}
    }

  replacement[j] = '\0';
  free (subst_rhs);
  subst_rhs = replacement;
  subst_rhs_len = j;
}
	...
	case 's':
	  {
		...
		temp_event = get_subst_pattern
		  (string, &i, delimiter, 0, &subst_lhs_len);
		...
		if (temp_event)
		  {
		    if (subst_lhs)
		      free (subst_lhs);
		    subst_lhs = temp_event;
		  }
		else if (!subst_lhs)
		  {
		    if (search_string && *search_string)
		      {
			subst_lhs = strdup (search_string);
...
history_expand (char *hstring, char **output)
{
  ...
  if (hstring[0] == history_subst_char)
      ...
      for (i = 0; string[i]; i++)
	{
	  cc = string[i + 1];
          if (string[i] == history_expansion_char)
	    {
	      if (!cc || strchr (history_no_expand_chars, cc))
		continue;
#if defined (SHELL)
  ...
  for (passc = i = j = 0; i < l; i++)
	...
	case -3:		/* history_expansion_char */
	  ...
	  if (strchr (history_no_expand_chars, cc))
	    {
	      ADD_CHAR (string[i]);
	      break;
	    }
...
get_history_word_specifier (char *spec, char *from, int *caller_index)
{
  ...
  if (spec[i] == '%')
    {
      *caller_index = i + 1;
      return (search_match ? strdup (search_match) : strdup (""));
    }
...
char *
history_arg_extract (int first, int last, char *string)
{
  register int i, len;
...
/* Parse STRING into tokens and return an array of strings.  If WIND is
   ...
   *INDP. */
static char **
history_tokenize_internal (char *string, int wind, int *indp)
{
  char **result = (char **)NULL;
  ...
  while (string[i])
      ...
      for (; string[i]; i++)
	{
	  ...
	  if (string[i] == '\\' && delimiter != '\'' &&
	      (delimiter != '"' || strchr (slashify_in_quotes, string[i])))
	    {
	      i++;
	      continue;
	    }
