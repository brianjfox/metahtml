...
extern void _rl_output_some_chars (char *string, int count);
extern int _rl_output_character_function (int c);
...
static void update_line (char *old_txt, char *new_txt, int current_line,
			 int omax, int nmax, int inv_botlin);
static void clear_to_eol (int count);
static void space_to_eol (int count);
static void delete_chars (int count);
static void insert_some_chars (char *string, int count);
...
static void
update_line (char *old_txt, char *new_txt, int current_line,
	     int omax, int nmax, int inv_botlin)
{
  ...
  if (temp == screenwidth && term_xn && !_rl_horizontal_scroll_mode
    ...
    {
      if (new_txt[0])
	putc (new_txt[0], rl_outstream);
      else
	putc (' ', rl_outstream);
      _rl_last_c_pos = 1;		/* XXX */
      _rl_last_v_pos++;
      if (old_txt[0] && new_txt[0])
        old_txt[0] = new_txt[0];
    }
      
  /* Find first difference. */
  for (ofd = old_txt, nfd = new_txt;
       (ofd - old_txt < omax) && *ofd && (*ofd == *nfd);
       ofd++, nfd++)
    ;
  ...
  for (od = ofd - old_txt, oe = ofd; od < omax && *oe; oe++, od++);
  for (nd = nfd - new_txt, ne = nfd; nd < nmax && *ne; ne++, nd++);
  ...
  if (current_line == 0 && !_rl_horizontal_scroll_mode &&
      ...
      _rl_last_c_pos > 0 && (ofd - old_txt) >= lendiff && term_cr)
    {
      tputs (term_cr, 1, _rl_output_character_function);
  ...
  _rl_move_cursor_relative (ofd - old_txt, old_txt);
  ...
  if (lendiff > 0)
    {
	 ...
	 around on auto-wrapping terminals. */
	  ...
	  ((2 * temp) >= lendiff || term_IC) && (!term_xn || !gl))
	{
...
void
_rl_move_cursor_relative (int newpos, char *data)
{
  register int i;

  /* If we don't have to do anything, then return. */
  if (_rl_last_c_pos == newpos) return;

  /* It may be faster to output a CR, and then move forwards instead
     of moving backwards. */
  /* i == current physical cursor position. */
  i = _rl_last_c_pos - W_OFFSET(_rl_last_v_pos, visible_wrap_offset);
  if (CR_FASTER (newpos, _rl_last_c_pos) || (term_xn && i == screenwidth))
    {
  ...
  if (_rl_last_c_pos < newpos)
	...
	for (i = _rl_last_c_pos; i < newpos; i++)
  ...
  else if (_rl_last_c_pos != newpos)
    backspace (_rl_last_c_pos - newpos);
  _rl_last_c_pos = newpos;
}
...
static void
insert_some_chars (char *string, int count)
{
#if defined (__GO32__)
  int row, col, width;
  char *row_start;
