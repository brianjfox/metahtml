...
typedef char **CompleterFunction (char *, int, int);
typedef int qsort_func (const void *, const void *);
typedef void Ignorer (char **);
typedef char *EntryFunction (char *text, int state);
...
static int compare_strings (char **, char **);
static char *rl_strpbrk (char *, char *);
  ...
  if (rl_filename_completion_desired && rl_visible_stats)
      ...
      if (to_print != full_pathname)
	{
	  ...
	  if (rl_directory_completion_hook)
	    (*(CFunction *)rl_directory_completion_hook) (&s);
...
rl_complete_internal (int what_to_do)
{
  ...
  if (rl_attempted_completion_function)
    {
      matches = (*(CompleterFunction *)rl_attempted_completion_function)
	(text, start, end);
  ...
  matches = completion_matches (text, (Function *)our_func);
  ...
  if (!matches)
      ...
      if (rl_ignore_completion_duplicates)
	{
	  ...
	  if (i)
	    qsort (matches+1, i-1, sizeof (char *),
		   (qsort_func *)compare_strings);

	  /* Remember the lowest common denominator for it may be unique. */
	  lowest_common = strdup (matches[0]);
	...
	case '!':
	  ...
	  if (rl_ignore_some_completions_function &&
	      our_func == (Function *)filename_completion_function)
	    ((Ignorer *)rl_ignore_some_completions_function) (matches);
	...
	case '?':
	  ...
	  display_matches:
	    ...
	    if (!rl_ignore_completion_duplicates)
	      qsort (matches + 1, len - 1, sizeof (char *),
		     (qsort_func *)compare_strings);
...
username_completion_function (char *text, int state)
{
  ...
  if (!state)
	...
	first_char_loc = 0;
      ...
      username = strdup (&text[first_char_loc]);
 ...
 */
char **
completion_matches (char *text, Function *fun)
{
  EntryFunction *entry_function = (EntryFunction *)fun;
...
filename_completion_function (char *text, int state)
{
  ...
  if (!state)
	...
	replace_dirname = 0;
	if (rl_directory_completion_hook)
	  replace_dirname = (*(CFunction *)rl_directory_completion_hook)
	    (&dirname);
	if (replace_dirname)
	  {
	    free (users_dirname);
...
static char *
rl_strpbrk (char *string1, char *string2)
{
  register char *scan;
