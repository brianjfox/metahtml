...
static int qsort_string_compare (char **s1, char **s2);
...
static FUNMAP default_funmap[] = {
 ...
 {(char *)NULL, (__readline_function *)NULL }
};

int
rl_add_funmap_entry (char *name, __readline_function *function)
{
  if (funmap_entry + 2 >= funmap_size)
	...
	{
	  funmap = (FUNMAP **)xrealloc
	    ((char *)funmap, (funmap_size += 80) * sizeof (FUNMAP *));
	}
    }
...
rl_funmap_names (void)
{
  ...
  qsort (result, result_index, sizeof (char *),
	 (qsort_func *)qsort_string_compare);
  return (result);
}
