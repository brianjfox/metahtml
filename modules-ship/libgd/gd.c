...
static void gdImageBrushApply(gdImagePtr im, int x, int y)
{
	...
	_y1 = y - hy;
	_y2 = _y1 + gdImageSY(im->brush);	
	hx = gdImageSX(im->brush)/2;
	_x1 = x - hx;
	_x2 = _x1 + gdImageSX(im->brush);
	srcy = 0;
	for (ly = _y1; (ly < _y2); ly++) {
		srcx = 0;
		for (lx = _x1; (lx < _x2); lx++) {
			int p;
...
void gdImageLine (gdImagePtr im, int x1, int my_y1, int x2, int y2, int color)
{
...
void gdImageDashedLine(gdImagePtr im, int x1, int my_y1, int x2, int y2, int color)
{
	int dx, dy, incr1, incr2, d, x, y, xend, yend, xdirflag, ydirflag;
	int dashStep = 0;
	int on = 1;
...
{
 unsigned int perrep;
 unsigned int cost_result;

 cost_result = 0;
 perrep = (nrepcodes * (nrepcodes+1)) / 2;
 while (count >= perrep)
  { cost_result += nrepcodes;
    count -= perrep;
  }
 if (count > 0)
  { unsigned int n;
...
void gdImageRectangle(gdImagePtr im, int x1, int my_y1, int x2, int y2, int color)
{
	gdImageLine(im, x1, my_y1, x2, my_y1, color);		
	gdImageLine(im, x1, y2, x2, y2, color);		
	gdImageLine(im, x1, my_y1, x1, y2, color);
	gdImageLine(im, x2, my_y1, x2, y2, color);
}

void gdImageFilledRectangle(gdImagePtr im, int x1, int my_y1, int x2, int y2, int color)
{
	int x, y;
	for (y=my_y1; (y<=y2); y++) {
		for (x=x1; (x<=x2); x++) {
...
void gdImageCopyResized(gdImagePtr dst, gdImagePtr src, int dstX, int dstY, int srcX, int srcY, int dstW, int dstH, int srcW, int srcH)
{
	...
	for (i=0; (i < srcW); i++) {
		int got;
		my_accum += (double)dstW/(double)srcW;
		got = floor(my_accum);
		stx[i] = got;
		my_accum -= got;
	}
	...
	for (i=0; (i < srcH); i++) {
		int got;
		my_accum += (double)dstH/(double)srcH;
		got = floor(my_accum);
		sty[i] = got;
		my_accum -= got;
	}
...
void gdImageFilledPolygon(gdImagePtr im, gdPointPtr p, int n, int c)
{
  ...
  if (im->polyAllocated < n)
	...
	(im->polyInts, sizeof(int) * im->polyAllocated);
    }
  ...
  for (y=my_y1; (y < y2); y++)
      ...
      for (i=0; (i <= n); i++)
	{
	  ...
	  if ((y >= my_my_y1) && (y <= my_y2))
	    {
	      int inter = 
		(y-my_my_y1) * (x2-x1) / (my_y2-my_my_y1) + x1;
...
void gdImageSetBrush(gdImagePtr im, gdImagePtr brush)
{
	...
	for (i=0; (i < gdImageColorsTotal(brush)); i++) {
		int ind;
		ind = gdImageColorExact(im, 
			gdImageRed(brush, i),
			gdImageGreen(brush, i),
			gdImageBlue(brush, i));
		if (ind == (-1)) {
			ind = gdImageColorAllocate(im,
				gdImageRed(brush, i),
				gdImageGreen(brush, i),
				gdImageBlue(brush, i));
			if (ind == (-1)) {
				ind = gdImageColorClosest(im,
					gdImageRed(brush, i),
					gdImageGreen(brush, i),
					gdImageBlue(brush, i));
			}
		}
		im->brushColorMap[i] = ind;
	}
}
...
void gdImageSetTile(gdImagePtr im, gdImagePtr tile)
{
  ...
  for (i=0; (i < gdImageColorsTotal(tile)); i++)
    {
    int idx;
    idx = gdImageColorExact
      (im, gdImageRed(tile, i), gdImageGreen(tile, i), gdImageBlue(tile, i));
    if (idx == (-1))
      {
	idx = gdImageColorAllocate
	  (im, gdImageRed(tile, i), gdImageGreen(tile, i), gdImageBlue(tile, i));
      if (idx == (-1))
	{
	  idx = gdImageColorClosest
	    (im, gdImageRed(tile, i), gdImageGreen(tile, i), gdImageBlue(tile, i));
      }
    }
    im->tileColorMap[i] = idx;
  }
}
